...
export function DialogueScriptForm() {
  ...
  const[dialogue,setDialogue] = useState("Generated Script Output");
  return (
    <Card className="col-span-1 md:col-span-1 lg:col-span-1">
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea onChange={(e) => setPrompt(e.target.value) } id="podcast-prompt" placeholder="e.g., Discuss the future of AI in education" />
        </div>
        <div>
        ...
        <Button onClick={async () => {
          try {
            ...
            if(!response.ok){
              setDialogue("error occured");
              return
            }
            const data = await response.json();
            setDialogue(data.response);
          } catch (error) {
            console.log(error)
          }
          
        }} className="w-full">Generate Script</Button>
